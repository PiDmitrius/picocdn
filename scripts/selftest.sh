...
# ---- index_file ----
step "seed default/index.html and default/welcome/index.html"
...
if curl -fsS -X PUT --data-binary @"$WORK/root.html" -H "Authorization: Bearer $OWNER_TOKEN" \
   ...
   && curl -fsS -X PUT --data-binary @"$WORK/welcome.html" -H "Authorization: Bearer $OWNER_TOKEN" \
     -H 'Content-Type: text/html' "$HOST/welcome/index.html" >/dev/null 2>&1; then ok
else ko "PUT failed"; fi

step "POST /_/namespaces/default/index sets index_file"
out=$(admin_call POST "$ADMIN/default/index" "$OWNER_TOKEN" '{"file":"index.html"}' 2>&1)
if echo "$out" | grep -q '"index_file":"index.html"'; then ok; else ko "$out"; fi

step "anon GET /default/ returns the root index"
admin_call POST "$ADMIN/default/public" "$OWNER_TOKEN" '{"on":true}' >/dev/null
body=$(curl -fsS "$HOST/" 2>/dev/null)
if [[ "$body" == *"root"* ]]; then ok; else ko "got: $body"; fi

step "anon GET /default/welcome/ returns the subdir index"
body=$(curl -fsS "$HOST/welcome/" 2>/dev/null)
if [[ "$body" == *"welcome"* ]]; then ok; else ko "got: $body"; fi

step "anon GET /default (no slash) also serves root index"
body=$(curl -fsS "$HOST" 2>/dev/null)
if [[ "$body" == *"root"* ]]; then ok; else ko "got: $body"; fi
...
step "POST /_/namespaces/default/index clears index_file"
out=$(admin_call POST "$ADMIN/default/index" "$OWNER_TOKEN" '{"file":""}' 2>&1)
if echo "$out" | grep -q '"index_file":""'; then ok; else ko "$out"; fi

# Restore public_read=off and seed back the deleted hello.txt-like file is
# not necessary — the subsequent sections do not rely on default/index.html.
admin_call POST "$ADMIN/default/public" "$OWNER_TOKEN" '{"on":false}' >/dev/null
